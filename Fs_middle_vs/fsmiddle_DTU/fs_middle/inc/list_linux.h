...
#else
extern void __list_add(struct list_head *item,
			      struct list_head *prev,
			      struct list_head *next);
#endif
...
// #define list_entry(list_head, typ, member_name) \
// 	(typ *)((unsigned int)list_head - (unsigned int)(&(((typ*)(0))->member_name)))  
// 
// 
...
#define list_entry(ptr, type, member_name) \
	(type *)((unsigned int)ptr - (unsigned int)(&(((type*)(0))->member_name)))  
...
/**
 * list_here_each	-	iterate over a list from current position
 * @pos:	the &struct list_head to use as a loop cursor.
 * @head:	the head for your list.
 */
#define list_here_each(pos, head) \
	for (; pos != (head); \
        	pos = pos->next)
...
/**
 * list_here_each_prev	-	iterate over a list backwards from current position
 * @pos:	the &struct list_head to use as a loop cursor.
 * @head:	the head for your list.
 */
#define list_here_each_prev(pos, head) \
	for (; pos != (head); \
        	pos = pos->prev)
